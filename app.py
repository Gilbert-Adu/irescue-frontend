...
from bot import message_clients
...
def delete_task_by_id(id):
    try:
        ...
        if listings:
            ...
            print("listings_urls: ", LISTING_URLS)
            
            messagePayload = messagesTable.scan(
                FilterExpression=boto3.dynamodb.conditions.Attr('product_url').eq(LISTING_URLS[0])
                )
...
def teach_bot(question, answer):
    ...
    url = "https://irescue-ngrok.ngrok.io/train_bot"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("successfully trained the bot with a question and answer")

        else:
            print("Failed to train bot. Response was bad.")


    except requests.exceptions.RequestException as e:
        print("Error while training bot: ", str(e))
...
def login_to_facebook(city, query, condition, minPrice, maxPrice, brandInput, product, networkInput):
    ...
    try:
        ...
        print(f"An error occurred: {e}")
    print("matches: ", scrape_listings(query, int(minPrice), int(maxPrice), url))
...
def start_background_function():
    threading.Thread(target=message_clients, daemon=True).start()
